...
import eu.toldi.bpmn_zkp.model.bpmn.ExclusiveGateway
import eu.toldi.bpmn_zkp.model.bpmn.ExclusiveGatewayEnd
import eu.toldi.bpmn_zkp.model.bpmn.ExclusiveGatewayStart
import eu.toldi.bpmn_zkp.model.bpmn.Transition
import eu.toldi.bpmn_zkp.model.helper.TagNames
import org.w3c.dom.Node

class ExclusiveGatewayParser : ElementParser {
    override fun parse(model: Model, element: Node): ExclusiveGateway {
        ...
        if (toId.size > 1 && tiId.size == 1) {
            val default = try {
                model.getTransition(element.attributes.getNamedItem("default").textContent)
            } catch (e: Exception) {
                null
            }
            return ExclusiveGatewayStart(name, transitions_in[0], transitions_out, default).apply {
            ...
            return ExclusiveGatewayEnd(name, transitions_in, transitions_out[0]).apply {
